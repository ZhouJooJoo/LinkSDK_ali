...
#include "utils/core_mqtt.h"
/*完成函数实现*/
#include "aiot_mqtt_api.h"
#include "utils/core_log.h"
#include "utils/core_diag.h"
#include "utils/core_string.h"
#include "utils/core_auth.h"
#include "utils/core_global.h"
        ...
        core_mqtt_event_t *core_event)
{
    ...
    /*node是在函数里面定义的一个初始化的值，真正要循环的是 &mqtt_handle->process_data_list*/
...
//客户端执行一个操作（如订阅、发布或取消订阅）时，该字段的值就会增加1，另一个函数负责字段值-1
...
//添加新的clientID,channel_handle是mqtt的客户端句柄，dst_clientid指向的指针将用于存储修改后的客户端ID，extend是字符串，新的id
static int32_t _core_mqtt_add_extend_clientid(core_mqtt_handle_t *channel_handle, char **dst_clientid, char *extend)
{
	...
	//如果原本dst_clientid为空，原始客户端ID还没有存储
    ...
    if (mqtt_handle->cred != NULL) {
	   ...
	   //需要使用服务器证书和预共享密钥（PSK）来建立安全连接
        if (mqtt_handle->cred->option == AIOT_SYSDEP_NETWORK_CRED_SVRCERT_PSK) {
            ...
            core_log1(mqtt_handle->sysdep, STATE_MQTT_LOG_TLS_PSK, "%s\r\n", sysdep_psk.psk);
			//将 sysdep_psk 结构体的信息设置到 MQTT 客户端的网络句柄 mqtt_handle->network_handle 中
			//指定了选项 CORE_SYSDEP_NETWORK_PSK。这一步是为了将预共享密钥（PSK）信息配置到网络连接中，以便建立安全连接
...
//向管理MQTT客户端处理数据的数据结构（process_data_list）中插入新的数据节点
        ...
        core_mqtt_process_data_t *process_data)
{
    ...
    core_list_for_each_entry(node, &mqtt_handle->process_data_list,
                             linked_node, core_mqtt_process_data_node_t) {
     	//对比每个节点的 handler 字段与传入的 process_data->handler是否一致
        if (node->process_data.handler == process_data->handler) {
            node->process_data.context = process_data->context;//一致的情况下更新上下文
            return STATE_SUCCESS;
        }
    }

    if (&node->linked_node == &mqtt_handle->process_data_list) {//遍历完整个链表都没有找到相同的处理函数，说明需要创建一个新的处理数据节点
...
//从 MQTT 数据流中读取一个字节一个字节的字符，并使用特定规则计算剩余长度
static int32_t _core_mqtt_read_remainlen(core_mqtt_handle_t *mqtt_handle, uint32_t *remainlen)
{
    int32_t res = 0;
    uint8_t ch = 0;
...
//从 MQTT 数据流中读取指定长度的数据，通常用于解析 MQTT 消息的内容部分，例如消息体或有效载荷
...
//处理 MQTT 协议中的 PINGRESP（Ping Response）消息
static int32_t _core_mqtt_pingresp_handler(core_mqtt_handle_t *mqtt_handle, uint8_t *input, uint32_t len)
{
    ...
    uint64_t rtt = mqtt_handle->sysdep->core_sysdep_time()  \
                   - mqtt_handle->heartbeat_params.last_send_time;//round trip time
...
//publish acknowledgement信息发送。在MQTT 服务端的代码中被调用
...
static void _core_mqtt_call_user_handler(core_mqtt_handle_t *mqtt_handle, core_mqtt_msg_t msg, uint8_t qos)
{
    ...
    memset(&packet, 0, sizeof(aiot_mqtt_recv_t));
    packet.type = AIOT_MQTTRECV_PUB;//设置为publish报文的格式
    ...
    core_list_for_each_entry(sub_node, &mqtt_handle->sub_list, linked_node, core_mqtt_sub_node_t) {
        if (_core_mqtt_topic_compare(sub_node->topic, (uint32_t)(strlen(sub_node->topic)), packet.data.pub.topic,
            ...
            //复制sub_node中的处理程序节点，复制到handler_list_copy链表末尾，保证copy链表包含所有与接收到的消息主题匹配的处理程序节点
    ...
    core_list_for_each_entry(handler_node, &handler_list_copy,
                             linked_node, core_mqtt_sub_handler_node_t) {
                             //将相应的用户数据分配给 userdata 变量，如果处理程序节点的 userdata 为 NULL，则使用 MQTT 句柄中的默认用户数据
    ...
    /* User Data Default Packet Handler（如果没有找到匹配的主题） */
...
static int32_t _core_mqtt_pub_handler(core_mqtt_handle_t *mqtt_handle, uint8_t *input, uint32_t len, uint8_t qos)
{
    ...
    //utf8_strlen用于存储消息主题的长度,分别是两个字节
    utf8_strlen = input[idx++] << 8;
    utf8_strlen |= input[idx++];

    //是将消息主题的地址传递给 src.topic 变量
...
{
    aiot_mqtt_recv_t packet;//存储解析后的PUBACK消息数据。

    if (input == NULL || len != 2) {
        return STATE_MQTT_RECV_INVALID_PUBACK_PACKET;//PUBACK消息不符合规定格式
    }
    ...
    /* Remove Packet From republist 因为PUBACK消息用于确认PUBLISH消息的传递，并且一旦确认后，相关的PUBLISH消息可以从发布列表中移除*/
    mqtt_handle->sysdep->core_sysdep_mutex_lock(mqtt_handle->pub_mutex);
    ...
    if (mqtt_handle->recv_handler) {
        mqtt_handle->recv_handler((void *)mqtt_handle, &packet, mqtt_handle->userdata);//通知应用程序接收到了PUBACK消息。
    }
        ...
        uint8_t packet_type)
{
    ...
    for (idx = 0; idx < len;) {
        ...
        if (packet_type == CORE_MQTT_SUBACK_PKT_TYPE) {
            ...
            idx += 3;//将索引指向下一个订阅确认消息的位置（如果存在多个订阅确认消息）
        } else {
            idx += 2;
        }
...
void *aiot_mqtt_init(void)
{
    ...
    sysdep = aiot_sysdep_get_portfile();//初始化全部赋值为空指针
    if (sysdep == NULL) {
        return NULL;
    }
    ...
    memset(mqtt_handle, 0, sizeof(core_mqtt_handle_t));//初始化句柄值
